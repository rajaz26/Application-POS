...
const [scannedProduct, setScannedProduct] = useState({ name: '', quantity: 0 }); // State to store scanned product details
const [vendor, setVendor] = useState(route.params?.vendor || '');
  ...
  const [purchaseOrderAmount,setPurchaseOrderAmount]=route.params.purchaseOrderAmount;
      ...
      <View style={styles.modalContainer}>
        ...
        {scannedProducts.map((product, index) => (
          <Text key={index} style={styles.scannedText}>
            {`${product.productName} (Quantity: ${product.quantityOrdered})`}
          </Text>
        ))}
...
const handleSaveScannedProduct = () => {
    route.params.handleScannedProduct(scannedProduct); // Call handleScannedProduct function passed from PurchaseOrder screen
...
const handleAddQuantity = async () => {
    ...
    if (existingProductIndex !== -1) {
        ...
        const createPurchaseItemInput = {
            ...
            quantityReceived: 0,
            purchaseOrderPurchaseItemsId: route.params.purchaseOrderId,
            productPurchaseItemsId: currentProduct.id,
        };

        const createdPurchaseItem = await client.graphql({
            query: createPurchaseItem,
            variables: { input: createPurchaseItemInput },
            authMode: 'apiKey',
        });

        // Calculate subtotal and update the purchase order amount
        const subtotal = parseInt(quantity, 10) * currentProduct.price;
        updatedPurchaseOrderAmount += subtotal;
    ...
    console.log("PASSING", purchaseOrderId, refreshPurchaseOrder,  updatedPurchaseOrderAmount,)
    navigation.navigate('PurchaseOrder', {
        purchaseOrderId: route.params.purchaseOrderId,
        refreshPurchaseOrder: true, 
        purchaseOrderAmount: updatedPurchaseOrderAmount,
    });
};
...
useEffect(() => {
    console.log("Purcase", route.params.purchaseOrderItems);
    setScannedProducts(route.params.purchaseOrderItems);
